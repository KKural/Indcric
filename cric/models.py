...
class Payment(models.Model):
    ...
    session = models.ForeignKey(
        'Session', on_delete=models.CASCADE)  # Link to Session
    ...
    class Meta:
        # Ensure each user can only be paid once per session
...
class Poll(models.Model):
    session = models.OneToOneField(
        Session, on_delete=models.CASCADE, related_name='poll')
    question = models.CharField(
        max_length=255, default="Should this session be played?")
...
class Match(models.Model):
    session = models.ForeignKey(
        Session, on_delete=models.CASCADE, related_name='matches', null=True)
    name = models.CharField(max_length=100)
    winner = models.ForeignKey(
        'Team', on_delete=models.CASCADE, related_name='won_matches', null=True, blank=True)
